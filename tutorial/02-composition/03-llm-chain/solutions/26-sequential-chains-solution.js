...
function createTextCleaningChain() {
    return new TransformChain({
        ...
        transform: async ({ text }) => {
            ...
            // 1. Keep only allowed characters: . , ! ? - '
            //    Remove all others except letters, numbers, and spaces.
...
async function createSummaryChain() {
    ...
    const chain = new LLMChain({ prompt, llm, outputParser: parser, outputKey: "summary" });
...
function createKeywordExtractionChain() {
    const prompt = new PromptTemplate({
        template: "Extract 5 keywords from: {cleanedText}\nKeywords (comma-separated):",
        inputVariables: ["cleanedText"]
    });
    const parser = new StringOutputParser();

    const chain = new LLMChain({ prompt, llm, outputParser: parser, outputKey: "keywords" });
...
function createMetadataChain() {
    return new TransformChain({
        inputVariables: ["cleanedText", "keywords"],
        outputVariables: [],

        transform: async ({ cleanedText, keywords }) => {
...
async function createContentProcessingPipeline() {
    const cleaningChain = createTextCleaningChain();
    const summaryChain = await createSummaryChain();
    ...
    const sequential = new SequentialChain({
        chains: [cleaningChain, summaryChain, keywordChain, metadataChain],
        inputVariables: ["text"],
        outputVariables: ["cleanedText", "summary", "keywords", "metadata"]
    });
...
async function createOptimizedPipeline() {
    const cleaningChain = createTextCleaningChain();
    const summaryChain = await createSummaryChain();
    const keywordChain = createKeywordExtractionChain();
    const metadataChain = createMetadataChain();
    ...
    return {
        async invoke(inputs) {
            ...
            const cleanedResult = await cleaningChain.invoke({ text: inputs.text });
            const cleanedText = typeof cleanedResult === 'string' ? cleanedResult : cleanedResult.cleanedText;

            // Step 2 - Run summary and keyword extraction in parallel
            const [summaryResult, keywordResult] = await Promise.all([
                summaryChain.invoke({ cleanedText }),
                keywordChain.invoke({ cleanedText })
            ]);

            const summary = summaryResult.summary || summaryResult;
            const keywords = keywordResult.keywords || keywordResult;

            // Step 3 - Calculate metadata
            const metadataResult = await metadataChain.invoke({ cleanedText, keywords });
            const metadata = typeof metadataResult === 'object' && metadataResult.metadata ? metadataResult.metadata : metadataResult;
...
async function createConditionalPipeline() {
    const cleaningChain = createTextCleaningChain();
    const summaryChain = await createSummaryChain();
    const keywordChain = createKeywordExtractionChain();
    const metadataChain = createMetadataChain();
    ...
    return {
        async invoke(inputs) {
            // Clean the text
            const cleanedResult = await cleaningChain.invoke({ text: inputs.text });
            console.log('   DEBUG cleanedResult:', JSON.stringify(cleanedResult, null, 2));
            const cleanedText = typeof cleanedResult === 'string' ? cleanedResult : cleanedResult.cleanedText;
            ...
            if (wordCount < 50) {
                ...
                const summaryResult = await summaryChain.invoke({ cleanedText });
                summary = summaryResult.summary || summaryResult;
            }

            // Always extract keywords
            const keywordResult = await keywordChain.invoke({ cleanedText });
            const keywords = keywordResult.keywords || keywordResult;
...
console.log('🧪 Running Exercise 26 Tests...\n');
...
async function runTests() {
    ...
    try {
        ...
        const cleanedText = typeof result === 'string' ? result : result.cleanedText;
        
        if (!cleanedText) {
            throw new Error('Result should have cleanedText or be a string');
        }
    ...
    try {
        const chain = await createSummaryChain();
    ...
    try {
        ...
        const meta = typeof result === 'object' && result.metadata ? result.metadata : result;
        
        if (!meta) {
            throw new Error('Result should have metadata or be a metadata object');
        }
    ...
    try {
        const pipeline = await createContentProcessingPipeline();
    ...
    try {
        const pipeline = await createOptimizedPipeline();
    ...
    try {
        const pipeline = await createConditionalPipeline();
    ...
    if (failed === 0) {
        ...
        console.log('You have successfully completed Exercise 26.');
